...
const manifest = {
  ...
  "parameters": {
    ...
    "properties": {
      "action": {
        "type": "string",
        "description": "Operation mode: 'generate' (default), 'list_tools', 'upload_file', or 'list_history'. Use 'list_history' when the user asks about photos/pictures/images Yui has created before (e.g. 'pernah bikin foto apa saja', 'list my photos') — it returns a clean deduplicated list of past generations (date, prompt, model, file).",
        "default": "generate"
      },
      "prompt": {
        "type": "string",
      ...
      "limit": {
        "type": "integer",
        "description": "Max history items to return for action: 'list_history'.",
        "default": 20
      },
      "sendToChat": {
        "type": "boolean",
        "description": "Whether to auto-send the generated image directly to the user's chat when the download succeeds. Defaults to true in chat contexts (tg_/dc_). Set to false to return the local path only.",
        "default": true
      },
      "count": {
        "type": "integer",
        "description": "Number of images to generate in this call (1-4, default 1). Use 2-4 only when the user explicitly asks for multiple photos.",
        "default": 1
      }
    },
    "required": ["prompt"]
