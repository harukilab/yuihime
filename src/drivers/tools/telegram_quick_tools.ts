  ...
  },
  {
    ...
    handler: async (tc) => {
      ...
      try {
        ...
        const srow = (label: string, v: any, extra = '') =>
          `\n${label}: *${v ?? '—'}%* ${extra}\n    ${bar(v)}`;
        if (lv && (lv.hunger !== undefined || lv.thirst !== undefined || lv.sleepiness !== undefined)) {
          ...
          lifeSection =
            `\n\n*🧬 Life Simulation*` +
            srow('🍽️ Hunger', lv.hunger) +
            srow('💧 Thirst', lv.thirst) +
            srow('🚿 Cleanliness', lv.cleanliness) +
            srow('🚽 Pee', lv.pee) +
            srow('💩 Poop', lv.poop) +
            srow('😴 Sleepiness', lv.sleepiness) +
            `\n🛏️ Sleep: ${sleepDot} ${lv.sleepState === 'asleep' ? '_Asleep_' : '_Awake_'} | Schedule ${lv.effectiveBedtime || '—'}-${lv.effectiveWake || '—'}` +
            `\n🐟 Fish craving: *${lv.fishCraving ?? '—'}%* ${bar(lv.fishCraving)} | Play urge: *${lv.playUrge ?? '—'}%* ${bar(lv.playUrge)}` +
        ...
        const hasMood = (moodObj && Object.keys(moodObj).length > 0) ||
          (emotionObj && (emotionObj.arousal !== undefined || emotionObj.valence !== undefined || emotionObj.focus !== undefined));
        if (hasMood) {
          lifeSection += `\n\n*💖 Mood*\n${moodSummary(moodObj, emotionObj)}`;
        }
      } catch (e: any) {
