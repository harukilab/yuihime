    ...
    configSchema: {
      fields: {
        ...
        cognitivePrompt: {
          ...
          default: '[SANDBOX PHYSICAL REALITY]: You have complete system authorization to perform automatic file executions and manipulations in the active sandbox workspace using the "file_automation" toolset. The files currently residing in the workspace directory are:'
        }
      }
    }
  },
      ...
      if (autoEnable && textMatches) {
        ...
        if (res.ok) {
          ...
          if (fileList.length > 0) {
            ...
            const contextPayload = `${injectionPrefix}\n${formattedFiles}\n\nUse the "file_automation" tool directly if the user commands file sorting, compilation, compression (zipping), parsing, indexing, summaries, or type conversions.`;
