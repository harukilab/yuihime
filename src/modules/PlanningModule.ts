...
export const PlanningModule: CortexModule = {
  ...
  run: async (input, state, context) => {
    ...
    // Determine if planning is needed when explicitly requested OR automatically for complex prompts
    const requiresPlanning = explicitPlanning || (hasComplexWord && tokenCount > 8);
