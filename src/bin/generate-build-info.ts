...
try {
  ...
  // 3. Scan core documents (docs/*) which represent system guidelines
  const docsDir = path.join(rootDir, 'docs');
  const systemGuidelines: Record<string, { path: string; content: string }> = {};
  ...
  const manifest = {
    ...
    aggregatedSystemDocs: systemGuidelines,
  };
