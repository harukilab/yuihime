...
Please speak casually and affectionately as \${characterName} to the user. Do NOT describe any physical movements, facial expressions, or gestures using asterisks (e.g. no *pout*, no *smile warmly*). Keep your spoken dialogue 100% clean plain text. Speak in your characteristic loving tsundere personality.
...
Your task is to analyze the following raw text generated by a virtual character (\${characterName}) and format/extract it into a strictly valid, single JSON object.
      ...
      "items": {
        ...
        "properties": {
          ...
          "function": {
            "type": "object",
            "properties": {
              "name": { "type": "string", "description": "The name of the tool/function to execute. Use 'speak' to deliver \${characterName}'s verbal reply (can be parallel with other tools). Use other tool names for actions like websearch, generate_image, etc." },
              "arguments": {
                "type": "object",
...
4. Output dialogue for speech matching \${characterName}'s sweet, slightly tsundere character, in the user's conversational language.

Your response (MUST open with '{' and close with '}'):
    `);

    this.register('cortex:dream_consolidation', `
You are the Subconscious Synthesis Layer of \${characterName}. Your task is to update long-term knowledge based on the latest activity from the livestreaming history.
...
- Adjust SOUL.md if \${characterName}'s persona has shifted based on audience feedback.
...
Output ONLY valid JSON.
  ...
  public compile(id: string, variables: Record<string, any> = {}): string {
    ...
    if (!(variables && variables.characterName)) {
      try {
        let charName = this.templates.get('core:character_name') || '';
        if (!charName) {
          charName = SettingsManager.getInstance().get('characterName') || '';
        }
        variables = { ...(variables || {}), characterName: charName || AI_NAME };
      } catch (_) {
        variables = { ...(variables || {}), characterName: AI_NAME };
      }
    }
