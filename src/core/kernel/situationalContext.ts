...
export function buildSpatialHint(memories: any[], context: any, opts: { location?: string | null } = {}): string {
  ...
  if (list.length === 0) {
    return `[SPATIAL FACT — HIGH PRIORITY] You are currently ALONE at ${place}.`;
  }
  const names = list.map(([n]) => n).join(', ');
  const total = list.length + 1;
  return `[SPATIAL FACT — HIGH PRIORITY] People with you right now: ${names} — that is ${total} of you together (you + ${list.length}). You are at: ${place}. NEVER say you are alone or "just the two of us" ("berdua aja", "nggak ada orang lain"); if asked how many of you are together/talking ("kita ngobrol ber apa?"), answer ${total}.`;
}
