...
export class ApiKeyPool {
  ...
  public configure(providerId: string, config: any, settings: any = {}): void {
    const single = typeof config?.apiKey === 'string' && config.apiKey.trim() ? [config.apiKey.trim()] : [];
    const list = Array.isArray(config?.apiKeys)
      ? config.apiKeys.map((k: any) => String(k).trim()).filter(Boolean)
      : [];
