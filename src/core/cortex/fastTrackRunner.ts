      ...
      try {
        ...
        if (!this.fastTrackWorker) {
          ...
          this.fastTrackWorker.on('error', (err: any) => {
            console.error("[CORTEX:Worker:Error] Worker thread encountered an error:", err);
          ...
          this.fastTrackWorker.on('exit', (code: number) => {
            if (code !== 0) {
              console.warn(`[CORTEX:Worker:Exit] Worker thread exited unexpectedly (code ${code}) — will respawn on next call.`);
            }
            this.fastTrackWorker = null;
          });
        }
        ...
        this.workerIdleTimeout = setTimeout(() => {
          if (this.fastTrackWorker) {
            console.log("[CORTEX:Worker:Idle] Background worker shut down after 5 min idle — will respawn on demand.");
        ...
        console.warn("[CORTEX:FastTrack] Worker unavailable, running mood decay synchronously:", err?.message || err);
        ...
        try {
          ...
          if (db) {
            ...
            if (shouldWriteMood) {
              ...
              // throttled — skipping mood DB write to reduce I/O overhead
            }
          ...
          console.error("[CORTEX:FastTrack:DbError] Failed to persist background state updates:", dbErr);
        }
      })();
    }
