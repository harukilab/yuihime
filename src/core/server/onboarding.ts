...
export function runOnboarding() {
  ...
  const updateProgress = (step: number, total: number, message: string) => {
    ...
    process.stdout.write(`\r\x1b[35m${spinner} [YUIHIME BOOT] [${bar}] ${Math.round((step / total) * 100)}% | ${message}...\x1b[0m`);
  };
...
`;
  ...
  if (isInteractive) {
    ...
    console.log("ℹ️ Berjalan dalam mode non-interaktif atau tanpa terminal TTY (Setup TUI dilewati otomatis).");
  }
