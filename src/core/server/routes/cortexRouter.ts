  ...
  app.post("/api/cortex/think", async (req, res) => {
    try {
      ...
      if (result.fallbackTriggered) {
        console.log(`[API_THINK] Gateway fallback triggered for ${senderName} (${finalChatType}). Saving to offline queue (pending_messages)...`);
        try {
          ...
          console.error("[API_THINK_FALLBACK_ERR] Failed to save fallback message to database:", dbErr.message);
        }
        const auditLogs = APIService.getAuditLogs();
        if (shouldStream) {
          res.write(`data: ${JSON.stringify({ type: "done", result: { response: null, auditLogs, queued: true } })}\n\n`);
          res.end();
        } else {
          res.json({ success: true, result: { response: null, auditLogs, queued: true } });
        }
        return;
      }
