...
Dokumen panduan ini ditujukan bagi para pengembang (developer), modder, atau streamer yang ingin menghubungkan aplikasi, bot eksternal, web scrapper (Twitch, TikTok, Discord), maupun skrip otomasi (Python/Shell) langsung ke sirkuit kognitif cerdas Yuihime untuk melakukan aktivitas **Live Streaming**.
...
```javascript
async function sendStreamChat(sender, message, platform = "Twitch Chat") {
...
1.  **Akses Mandiri Panggung**: `http://localhost:3000/?mode=obs`
    ...
    *   Secga default berlatar belakang **Neon** yang menawan.
