...
echo -e "   System Root Data      : ${YELLOW}${BOLD}\$HOME/.yuihime${NC}"
echo -e "${BLUE}===============================================================${NC}\n"
...
inject_bashrc_aliases() {
    local BASHRC="$HOME/.bashrc"
    local PROJECT_DIR="$(cd "$(dirname "$0")" && pwd)"

    echo -e "\n${CYAN}[EXTRA] Menyuntikkan alias Yuihime ke ${BASHRC}...${NC}"

    declare -A ALIAS_MAP=(
        ["yuihime"]="YUIHIME_SYSTEM_ROOT=\"\$HOME/.yuihime\" npm run --prefix ${PROJECT_DIR} dev"
        ["yuihimebuild"]="YUIHIME_SYSTEM_ROOT=\"\$HOME/.yuihime\" npm run --prefix ${PROJECT_DIR} build:bin"
        ["yuihimestart"]="YUIHIME_SYSTEM_ROOT=\"\$HOME/.yuihime\" npm run --prefix ${PROJECT_DIR} start"
    )

    for alias_name in "${!ALIAS_MAP[@]}"; do
        if grep -q "^alias ${alias_name}=" "$BASHRC" 2>/dev/null; then
            echo -e "  » ${GREEN}Alias '${alias_name}' sudah ada, melewatkan.${NC}"
        else
            echo "alias ${alias_name}='${ALIAS_MAP[$alias_name]}'" >> "$BASHRC"
            echo -e "  » ${GREEN}Alias '${alias_name}' ditambahkan.${NC}"
        fi
    done
}
...
install_and_build_yuihime() {
    ...
    echo -e "  » ${YELLOW}Membuat direktori kerja terisolasi di \$HOME/.yuihime/...${NC}"
    mkdir -p "$HOME/.yuihime/data/"
    mkdir -p "$HOME/.yuihime/user_data/"
    mkdir -p "$HOME/.yuihime/agent/"
    mkdir -p "$HOME/.yuihime/addons/"
    mkdir -p "$HOME/.yuihime/models/"
    ...
    if [ ! -f ./.env ]; then
        ...
        echo -e "  » ${GREEN}.env dibuat dengan YUIHIME_SYSTEM_ROOT=\$HOME/.yuihime${NC}"
    else
        if grep -q "^YUIHIME_SYSTEM_ROOT=" .env; then
            sed -i "s|^YUIHIME_SYSTEM_ROOT=.*|YUIHIME_SYSTEM_ROOT=\$HOME/.yuihime|" .env
        else
            echo "YUIHIME_SYSTEM_ROOT=\$HOME/.yuihime" >> .env
        fi
        echo -e "  » ${GREEN}.env diperbarui dengan YUIHIME_SYSTEM_ROOT=\$HOME/.yuihime${NC}"
    fi
    ...
    echo -e "  1) ${BOLD}Mode Standard Full-Stack${NC}"
    ...
    echo -e "  2) ${BOLD}Mode Standalone Single-Binary Executable (Direkomendasikan)${NC}"
    ...
    read -p "Masukan pilihan metode anda [1 atau 2] (Default: 2): " -r BUILD_OPT
    BUILD_OPT=${BUILD_OPT:-2}
...
echo -e "${CYAN}${BOLD}================================================================${NC}\n"
echo -e "Yuihime siap menemani petualangan Anda! 💖"
echo -e "Berikut adalah opsi cara menghidupkan core kognitif batin saya:"
echo -e "\n1. ${BOLD}Menggunakan alias bashrc (restart terminal atau jalankan 'source ~/.bashrc'):${NC}"
echo -e "   - ${YELLOW}yuihime${NC}        : Jalankan dev server"
echo -e "   - ${YELLOW}yuihimebuild${NC}   : Kompilasi single binary (build:bin)"
echo -e "   - ${YELLOW}yuihimestart${NC}   : Jalankan production server"
echo -e "\n2. ${BOLD}Menggunakan Node.js langsung:${NC}"
echo -e "   Run: ${YELLOW}npm run start${NC} atau ${YELLOW}node dist/server.cjs${NC}"
echo -e "\n3. ${BOLD}Menggunakan Berkas Biner Tunggal Mandiri (jika memilih Opsi 2):${NC}"
echo -e "   Run: ${YELLOW}./bin/yuihime-core-linux${NC} (Bila dikompilasi di Linux x64)"
echo -e "\n4. ${BOLD}Melalui Systemd Service Daemon (bila dikonfigurasi):${NC}"
echo -e "   Run: ${YELLOW}sudo systemctl start yuihime${NC}"
echo -e "\nSemua konfigurasi statis tersimpan di folder ${YELLOW}\$HOME/.yuihime/data/config.toml${NC}."
