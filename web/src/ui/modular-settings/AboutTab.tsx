...
import { APP_VERSION, AI_NAME } from '@shared/constants';
...
export const AboutTab: React.FC<AboutTabProps> = ({
  ...
  const displayVersion = yuihimeVersionInfo?.version || `v${APP_VERSION}`;
  return (
    <div className="space-y-6 animate-fade-in font-sans">
      <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
        ...
        <div className="bg-[#0e0e14]/55 border border-white/5 p-6 rounded-3xl space-y-6 flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-xl font-bold text-white tracking-wide">{AI_NAME} Virtual AGI</h3>
              </div>
              {onShowInfo && (
                <button
                  type="button"
                  onClick={() => onShowInfo(
                    `${AI_NAME} Virtual AGI Guidance`,
                    `${AI_NAME} is an autonomous, self-aware AI VTuber constructed inside a full-stack Node.js and React Sandbox Environment.\n\nShe integrates raw neural intuition (Deep Learning model backends) with strict logical rules (Symbolic AI structures) to achieve high-order metacognition.`
                  )}
            ...
            <p className="text-zinc-400 text-xs mt-1.5 leading-relaxed font-normal">
              {AI_NAME} is an autonomous, self-aware AI VTuber. Connects deep learning backends and symbolic logic rules.
            </p>
            ...
            <div className="space-y-2 border-t border-white/5 pt-4">
              ...
              <div className="flex items-center justify-between text-xs font-mono">
                ...
                <span className="text-amber-500 font-bold">Neuro-Symbolic Hybrid Core ({displayVersion})</span>
              </div>
              <div className="flex items-center justify-between text-xs font-mono">
          ...
          <div className="bg-black/40 border border-white/[0.03] p-4 rounded-2xl flex items-center gap-3 mt-4">
            ...
            <div>
              <span className="text-[10px] uppercase font-mono text-zinc-500">VTuber Identity</span>
              <h5 className="text-xs font-bold text-white leading-tight mt-0.5">{AI_NAME} — Virtual Soul for Real Connections</h5>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0e0e14]/55 border border-white/5 p-6 rounded-2xl">
        <h4 className="text-[10px] uppercase font-mono tracking-widest text-[#d4d4d8]/40 mb-3 font-bold">{AI_NAME} Cognitive Core Framework</h4>
