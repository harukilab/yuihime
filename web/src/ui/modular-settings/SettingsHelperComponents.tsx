...
const cleanOpenAiPath = (val: any): any => {
  if (typeof val === 'string') {
    return val
      .replace(/^(?:\/root|\.\.\/\.\.\/root|\/home\/[^\/]+)\/\.yuihime\/user_data\/?/g, '')
      .replace(/^(?:\/root|\.\.\/\.\.\/root|\/home\/[^\/]+)\/\.yuihime\/?/g, '')
      .replace(/^\/root\/?/g, '');
  }
  if (Array.isArray(val)) {
    return val.map(cleanOpenAiPath);
  }
  if (val && typeof val === 'object') {
    const cleaned: Record<string, any> = {};
    for (const [k, v] of Object.entries(val)) {
      cleaned[k] = cleanOpenAiPath(v);
    }
    return cleaned;
  }
  return val;
...
export const AuditLogCard: React.FC<{ log: any }> = ({ log }) => {
  ...
  const displayToolName = log.toolName || log.tool || log.name || log.parameters?.tool || log.parameters?.name || 'Tool Operation';

  const cleanedParams = cleanOpenAiPath(log.parameters);
  const cleanedResponse = cleanOpenAiPath(log.response);

  const handleCopyRaw = () => {
    navigator.clipboard.writeText(JSON.stringify(cleanedResponse || log.error || {}, null, 2));
    setCopiedRaw(true);
    ...
    <div className="bg-black/55 border border-white/[0.04] rounded-2xl p-3 sm:p-4 space-y-3 hover:border-amber-500/20 transition-all font-sans text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/[0.04] pb-2 font-mono">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          ...
          <span className="text-amber-400 text-[10px] sm:text-[11px] font-bold font-mono tracking-wide">
            {displayToolName}
          </span>
          <span className="text-[8.5px] sm:text-[9.5px] text-[#818cf8] truncate max-w-[120px] sm:max-w-none">
            ({log.endpointPath || '/api/cortex/tools'})
          </span>
        </div>
        <div className="flex flex-wrap items-center gap-1.5 self-start sm:self-auto">
          ...
          <span className={`text-[8px] sm:text-[8.5px] px-1.5 sm:px-2 py-0.5 rounded-full font-bold uppercase tracking-wider shrink-0 bg-cyan-500/10 text-cyan-400 border border-cyan-500/15 font-mono`}>
            OpenAI Tool Standard
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <div className="space-y-1 text-left w-full min-w-0">
          <span className="text-[8.5px] sm:text-[9px] text-zinc-500 font-mono tracking-wider uppercase font-bold block truncate">Tool Call Arguments (Clean Path)</span>
        ...
        <div className="space-y-1 text-left w-full min-w-0">
          <span className="text-[8.5px] sm:text-[9px] text-zinc-500 font-mono tracking-wider uppercase font-bold block truncate">
            {isSuccess ? 'Tool Response Output' : 'Execution Error Trace'}
          </span>
          <pre className={`bg-[#050508]/85 border border-white/[0.02] rounded-xl p-2 sm:p-2.5 text-[9.5px] sm:text-[10px] ${isSuccess ? 'text-indigo-300' : 'text-rose-400'} overflow-x-auto max-h-[145px] font-mono whitespace-pre-wrap break-words leading-relaxed select-text scrollbar-thin`}>
            {isSuccess ? JSON.stringify(cleanedResponse, null, 2) : log.error}
          </pre>
        </div>
      </div>
      ...
      {isSuccess && (
        <div className="space-y-1 bg-[#050508]/45 border border-white/[0.02] p-2 sm:p-2.5 rounded-xl text-left w-full min-w-0">
          <div className="flex justify-between items-center gap-2 mb-1">
            <span className="text-[8.5px] sm:text-[9px] text-zinc-500 font-mono tracking-wider uppercase font-bold truncate">OpenAI Structured Payload</span>
            <button
