...
import { MoodState, EmotionState, UserRelation } from '@shared/include/types';
...
class AvatarErrorBoundary extends React.Component<{ children: React.ReactNode; resetKey: string; onCatch?: () => void }, { hasError: boolean; error: any }> {
  ...
  componentDidCatch(error: any, errorInfo: any) {
    console.error("[AvatarErrorBoundary] Avatar render error caught:", error, errorInfo);
    this.props.onCatch?.();
  }
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 bg-black/85 text-center text-white z-20 font-sans space-y-3 rounded-2xl">
          <p className="text-xs font-bold text-amber-400">Avatar Transitioning...</p>
          <p className="text-[10px] text-zinc-400 max-w-xs">{this.state.error?.message || "Switching avatar display mode."}</p>
          <button
            type="button"
            onClick={() => this.setState({ hasError: false, error: null })}
            className="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/30 text-[10px] rounded-lg font-mono cursor-pointer"
          >
            Reload Model
          </button>
        </div>
...
export const VTuberAvatar: React.FC<VTuberAvatarProps> = ({ 
  ...
  return (
    ...
    >
      ...
      {!loadError && (
        <AvatarErrorBoundary resetKey={modelUrl} onCatch={handleLoadError}>
