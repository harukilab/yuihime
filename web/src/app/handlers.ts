...
export function useAppHandlers(
  ...
  const getCortex = () => {
    if (!s.cortexRef.current) s.cortexRef.current = new Cortex();
    if (s.config) s.cortexRef.current.setConfig(s.config);
    return s.cortexRef.current;
  };
  ...
  const handleOptimize = async () => {
    ...
    try {
      const updated = await LearningEngine.optimize(getCortex(), s.memories, s.state);
  ...
  const handleDream = async (currentMemories: any = s.memories) => {
    ...
    try {
      ...
      const { reflections } = await DreamEngine.startCycle(getCortex(), s.state);
      s.setMemoriesAtLastDream(currentMemories.length);
      chat.addLog('agent', `[DREAM_REFLEX] ${reflections}`);
  ...
  const handleSimulateLive = async () => {
    ...
    try {
      const { selectedMessage, contextSummary, action, reasoning } =
        await LiveModeratorModule.moderateChatBatch(fakeMessages, s.state.currentLiveTopic || "General");
  ...
  const handleConsolidate = async () => {
    ...
    try {
      ...
      const consolidatedDreams = await getCortex().consolidateDreams(s.dreams);
  ...
  const handleExtractKnowledge = async () => {
    ...
    try {
      s.setIsThinking(true);
      const updatedKnowledge = await LearningEngine.extractKnowledge(getCortex(), s.memories, s.knowledge);
  ...
  const handleReflect = async () => {
    ...
    try {
      ...
      const result = await getCortex().think(
        ...
        s.identities,
        s.NEURAL_CORES.find((c: any) => c.id === s.state.activePersonaId),
  ...
  const handleThink = async (e?: React.FormEvent, customOverrideText?: string, isDelayedRun?: boolean) => {
    ...
    if (activeInput.trim() === '/consolidate') {
      ...
      await Consolidator.run(getCortex(), s.memories);
      return;
    }
    ...
    try {
      ...
      const result = await getCortex().think(
        ...
        undefined,
        currentAttachments,
        s.llmStreamingEnabled ? (chunkText: string) => {
          accumulatedResponse += chunkText;
          s.setActiveSubtitle(accumulatedResponse);
