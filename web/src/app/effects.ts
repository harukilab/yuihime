...
export function useAppEffects(
  ...
  useEffect(() => {
    ...
    if (s.pulseEnabled) {
      CortexApi.startPulse().catch(() => {});
    } else {
      CortexApi.stopPulse().catch(() => {});
    }
  }, [s.pulseEnabled]);

  useEffect(() => {
    const updateCircuits = async () => {
      try {
        const status = await CortexApi.getCircuitStatus();
        s.setNeuralCircuitStatus(status);
      } catch (e) {
        // ignore
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubRemoteMsg = socketService.on('remote_message_received', (data: any) => {
    ...
    const unsubRemoteResp = socketService.on('remote_response_sent', (data: any) => {
      const { reply, channel } = data || {};
      if (reply) {
  ...
  useEffect(() => {
    const neuralHeartbeat = setInterval(() => {
      ...
      // Dreams require significant new interaction material and run at most once per 24 hours
      const lastDreamTime = (s as any).lastDreamTimestamp || 0;
      const isDailyElapsed = Date.now() - lastDreamTime > 24 * 60 * 60 * 1000;
      if (newMemoriesCount >= 10 && isDailyElapsed) {
        console.log("[SYSTEM] Autonomous Daily Dream Cycle Triggered: 24h & Chat Memory Threshold Reached");
        (s as any).lastDreamTimestamp = Date.now();
        h.handleDream();
      }
  ...
  // Maintenance cycle
  useEffect(() => {
    const AUTONOMOUS_INTERVAL = 1000 * 60 * 60 * 12; // 12 hours
    const runMaintenance = async () => {
      if (s.state.status === 'sleeping') return;
      const newMemoriesCount = s.memories.length - s.memoriesAtLastDream;
      const lastDreamTime = (s as any).lastDreamTimestamp || 0;
      if (newMemoriesCount >= 10 && Date.now() - lastDreamTime > 24 * 60 * 60 * 1000) {
        chat.addLog('agent', "[SYSTEM] Initiating daily autonomous background dream cycle...");
        (s as any).lastDreamTimestamp = Date.now();
        await h.handleDream();
      }
      if (s.memories.length >= 20 && s.memories.length % 20 === 0) {
        chat.addLog('agent', "[SYSTEM] Indexing knowledge from accumulated interactions...");
        await h.handleExtractKnowledge();
      }
    };
    const interval = setInterval(runMaintenance, AUTONOMOUS_INTERVAL);
    return () => clearInterval(interval);
  }, [s.memories.length, s.memoriesAtLastDream, s.state.status]);
  ...
  useEffect(() => {
    ...
    const triggerIdleBehavior = async () => {
      ...
      if (timeSinceLastInput > IDLE_THRESHOLD && !s.isThinking && s.state.status === 'idle' && s.idleMonologueEnabled !== false) {
  ...
  useEffect(() => {
    ...
    const triggerReminderReaction = async () => {
      ...
      try {
        ...
        const currentActivePersona = s.NEURAL_CORES.find((c: any) => c.id === s.state.activePersonaId);
        const result = await CortexApi.think({
          input: `[SYSTEM_SIGNAL]: A reminder just popped up: ${lastMsg.content}. Acknowledge it!`,
          memories: s.memories,
          dreams: s.dreams,
          capabilities: s.capabilities,
          state: s.state,
