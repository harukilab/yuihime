...
\${toolMeta}
`.trim();c.register("prompt-manager:available_tools",h),Ee=!0}const Qe={metadata:{id:"prompt-manager",name:"yui-cognition: Prompt Manager",description:"Consolidates system prompt, character lore, and context into a unified LLM instruction.",version:"1.2.0",type:Ye.CORTEX,phase:"PHASE 2: COMPRESSION",order:5,configSchema:{fields:{systemPrompt:{type:"textarea",label:"System Prompt Override",default:N,description:"Base instruction for the AI behavior."},characterLore:{type:"textarea",label:"Character Lore",default:E,description:"Personality and backstory."},worldLore:{type:"textarea",label:"World Knowledge",default:S,description:"Facts and world context."},dialogueContextSize:{type:"slider",label:"Conversation History Window",default:40,min:10,max:100,description:"Number of latest conversation memory records fed into the LLM neural core."},llmSizePreset:{type:"select",label:"LLM Multi-Tier Parameter Optimization Preset",default:"standard",options:[{value:"standard",label:"Standard - Full Cognitive Metacognition (High Param LLMs: >14B)"},{value:"medium",label:"Medium - Balanced CoT Flow (Medium Param LLMs: 7B - 14B)"},{value:"lite",label:"Lite - Compressed Context Window (Small Param LLMs: 2B - 4B)"},{value:"tiny",label:"Tiny - Direct Response & Ultra-Short Prompting (Tiny LLMs: <1.5B)"}],description:"Optimizes cognitive circuit parameters, conversation history size, prompt layout, JSON schema, and core data sent to the LLM based on parameter size to reduce latency and prevent cognitive timeouts."}}}},run:async(h,o,e)=>{var Z,ee,te,ae,ie,re,oe,ne,se,ce,le,me,de,ue,pe,he,fe,ye;console.log("[PROMPT_MANAGER] Assembling final instruction set with realistic growth metrics..."),await je();let r={};try{r=await R.getModularSettings()||{}}catch{}const m=e.moduleConfig||(r==null?void 0:r["prompt-manager"])||{},M=m.systemPrompt||c.get("core:system_prompt"),v=m.characterLore||c.get("core:character"),Y=m.worldLore||c.get("core:lore"),d=c.get("core:character_name")||F(v),A=(M||"").replace(/\$\{characterName\}/g,d);c.register("core:system_prompt",A,!0),c.register("core:character",v,!0),c.register("core:lore",Y,!0),c.register("core:character_name",d,!0);let f=[],p=[],w=[],K=[],W=[];try{f=e.memories||await R.getMemories()||[]}catch{}try{p=e.allIdentities||await R.getIdentities()||[]}catch{}try{w=e.dreams||await R.getDreams()||[]}catch{}try{K=e.heuristics||await R.getStrategies()||[]}catch{}try{W=await R.getCapabilities()||[]}catch{}const X=f.length>0?[...f].sort((t,a)=>t.timestamp-a.timestamp)[0]:null,we=X?X.timestamp:Date.now()-1e3*60*60*24*3.5,ve=Math.max(.1,Number(((Date.now()-we)/(1e3*60*60*24)).toFixed(1))),Oe=f.length,J=f.filter(t=>t.speaker&&t.speaker!=="agent"&&t.speaker!=="System"&&t.speaker!=="subconscious").length,Re=f.filter(t=>t.speaker==="agent").length,H=["Web Console UI"];((Z=r==null?void 0:r["telegram-bridge"])!=null&&Z.botToken||(ee=r==null?void 0:r["telegram-bridge"])!=null&&ee.enableTelegram)&&H.push("Telegram Bridge Platform"),((te=r==null?void 0:r["discord-bridge"])!=null&&te.token||(ae=r==null?void 0:r["discord-bridge"])!=null&&ae.enableDiscord)&&H.push("Discord Guild Server"),((ie=r==null?void 0:r["twitch-bridge"])!=null&&ie.oauthToken||(re=r==null?void 0:r["twitch-bridge"])!=null&&re.enableTwitch)&&H.push("Twitch Streaming Chat");const Se=((oe=o.relation)==null?void 0:oe.trust)||50,Ne=((ne=o.relation)==null?void 0:ne.affection)||50,Ae=W.filter(t=>t.enabled).length;let C=[];if(typeof window>"u")try{const t=typeof import.meta<"u"&&import.meta.url?import.meta.url:"";let a,i;if(t){const{createRequire:s}=await _(async()=>{const{createRequire:l}=await import("module");return{createRequire:l}},[]),u=s(t);a=u("fs"),i=u("path")}else typeof require<"u"?(a=require("fs"),i=require("path")):(a=await _(()=>import("fs"),[]),i=await _(()=>import("path"),[]));const n=i.resolve(process.cwd(),"src","core","available_tools.json");if(a.existsSync(n)){const s=a.readFileSync(n,"utf8");C=JSON.parse(s).map(u=>({metadata:u}))}}catch(t){console.warn("[PromptManager] Failed loading available_tools.json:",t)}(!C||C.length===0)&&(C=D.getTools());let L="";if(C.length>0)if(Array.isArray(e.tools)&&e.tools.length>0)L='Native tool calling is active. Tool schemas are provided via the standard API tools array. Use the standard `tool_calls` JSON format with `id`, `type: "function"`, and `function: { name, arguments }` structure.\n';else for(const a of C)L+=`- **${a.metadata.id}**: ${a.metadata.description}
`,a.metadata.parameters&&(L+=`  - Parameter Schema: \`\`\`json
${JSON.stringify(a.metadata.parameters,null,2)}
\`\`\`
`);else L="No external system tools are currently available.";const Ce=c.compile("prompt-manager:available_tools",{toolsList:L,toolSyntax:c.compile("tools:syntax_openai",{}),toolPagination:c.compile("tools:syntax_pagination",{}),toolOutput:c.compile("tools:output_format",{}),toolMeta:c.compile("tools:_meta",{})}),$e=D.getCortexModules(),be=D.getProviders(),Me=D.getTTSModules(),Le=D.getGateways();$e.map(t=>{var a,i,n,s;return`- **${((a=t.metadata)==null?void 0:a.id)||"unknown"}** (${((i=t.metadata)==null?void 0:i.name)||"Unnamed Module"} - Phase: ${((n=t.metadata)==null?void 0:n.phase)||"Unknown"}): ${((s=t.metadata)==null?void 0:s.description)||"No description"}`}).join(`
`),be.map(t=>{var a,i,n,s,u;return`- **${((a=t.metadata)==null?void 0:a.id)||"unknown"}** (${((i=t.metadata)==null?void 0:i.name)||"Unnamed Provider"} - Models: ${((s=(n=t.metadata)==null?void 0:n.models)==null?void 0:s.join(", "))||"Auto"}): ${((u=t.metadata)==null?void 0:u.description)||"No description"}`}).join(`
`),Me.map(t=>{var a,i,n;return`- **${((a=t.metadata)==null?void 0:a.id)||"unknown"}** (${((i=t.metadata)==null?void 0:i.name)||"Unnamed TTS"}): ${((n=t.metadata)==null?void 0:n.description)||"No description"}`}).join(`
`),Le.map(t=>{var a,i,n;return`- **${((a=t.metadata)==null?void 0:a.id)||t.id||"unknown"}** (${((i=t.metadata)==null?void 0:i.name)||t.name||"Unnamed Gateway"}): ${((n=t.metadata)==null?void 0:n.description)||t.description||"No description"}`}).join(`
`);const P=e.activePersona;let B="";P&&P.systemPrompt&&(B=`
# ACTIVE COGNITIVE FOCUS (${P.name||P.id})
${P.systemPrompt}
`);const I=m.llmSizePreset||"standard";let O=Number(m.dialogueContextSize||40);I==="tiny"?O=Math.min(8,O):I==="lite"?O=Math.min(15,O):I==="medium"&&(O=Math.min(30,O));const Q=f.filter(t=>t&&t.content&&t.content.trim().length>0&&(t.speaker||t.type==="dialogue"||t.type==="interaction")).sort((t,a)=>(t.timestamp||0)-(a.timestamp||0)).slice(-O),Pe=Q.length>0?Q.map(t=>{var i;let a=t.speaker||t.type;return a==="agent"?a=d:(a==="user"||!a||a==="chat"||a==="interaction")&&(a=e.userName&&e.userName!=="chat"&&e.userName!=="anon"?e.userName:((i=e.viewerIdentity)==null?void 0:i.perceivedName)||"user"),`${a}: ${t.content}`}).join(`
`):"No previous conversation records yet.";let V="",$=[];if(I==="tiny"?$=[{name:"IDENTITY.md",title:`WHO AM I (${d.toUpperCase()}'S IDENTITY)`,maxChar:500},{name:"USER.md",title:"WHO YOU ARE HELPING (HUMAN RELATIONSHIP DETAIL)",maxChar:500}]:I==="lite"?$=[{name:"IDENTITY.md",title:`WHO AM I (${d.toUpperCase()}'S IDENTITY)`,maxChar:1200},{name:"SOUL.md",title:`WHO YOU ARE (${d.toUpperCase()}'S SOUL & CHARACTER VALUE)`,maxChar:1e3},{name:"USER.md",title:"WHO YOU ARE HELPING (HUMAN RELATIONSHIP DETAIL)",maxChar:1e3}]:I==="medium"?$=[{name:"IDENTITY.md",title:`WHO AM I (${d.toUpperCase()}'S IDENTITY)`,maxChar:2500},{name:"SOUL.md",title:`WHO YOU ARE (${d.toUpperCase()}'S SOUL & CHARACTER VALUE)`,maxChar:2e3},{name:"MEMORY.md",title:"LONG-TERM MEMORY (CURATED EXPERIENCE & PREFERENCES)",maxChar:1500},{name:"USER.md",title:"WHO YOU ARE HELPING (HUMAN RELATIONSHIP DETAIL)",maxChar:1500}]:$=[{name:"IDENTITY.md",title:`WHO AM I (${d.toUpperCase()}'S IDENTITY)`},{name:"SOUL.md",title:`WHO YOU ARE (${d.toUpperCase()}'S SOUL & CHARACTER VALUE)`},{name:"MEMORY.md",title:"LONG-TERM MEMORY (CURATED EXPERIENCE & PREFERENCES)"},{name:"USER.md",title:"WHO YOU ARE HELPING (HUMAN RELATIONSHIP DETAIL)"},{name:"TOOLS.md",title:"LOCAL ENVIRONMENT NOTES & TOOL USAGE SPECIFICS"},{name:"HEARTBEAT.md",title:"PERIODIC FOCUSES & BACKGROUND TASKS"}],typeof window>"u")try{const t=typeof import.meta<"u"&&import.meta.url?import.meta.url:"";let a,i;if(t){const{createRequire:l}=await _(async()=>{const{createRequire:g}=await import("module");return{createRequire:g}},[]),y=l(t);a=y("fs"),i=y("path")}else typeof require<"u"?(a=require("fs"),i=require("path")):(a=await _(()=>import("fs"),[]),i=await _(()=>import("path"),[]));const n=T.YUIHIME_SYSTEM_ROOT||T.YUIHIME_ROOT||"~/.yuihime",s=i.isAbsolute(n)?n:i.join(process.cwd(),n),u=T.YUIHIME_AGENT_PATH||i.join(s,"agent");for(const l of $){let y=i.join(u,l.name);if(a.existsSync(y)||(y=i.join(process.cwd(),l.name)),a.existsSync(y),a.existsSync(y)){let g=a.readFileSync(y,"utf8").trim();l.maxChar&&g.length>l.maxChar&&(g=g.substring(0,l.maxChar)+`
...[Content truncated for tiny/lite model optimization Presets]...
`),g.length>0&&(V+=`
# ${l.title} (${l.name})
${g}
`)}}}catch(t){console.warn("[PROMPT_MANAGER] Dynamic markdown injections error:",t)}else try{const t=$.map(async i=>{try{const n=await fetch(`/api/system/markdown/${i.name}`);if(n.ok){const s=await n.json();if(s&&s.content&&s.content.trim().length>0){let u=s.content.trim();return i.maxChar&&u.length>i.maxChar&&(u=u.substring(0,i.maxChar)+`
...[Content truncated for tiny/lite model optimization Presets]...
`),`
# ${i.title} (${i.name})
${u}
`}}}catch(n){console.warn(`[PROMPT_MANAGER] Failed to fetch client-side markdown for ${i.name}:`,n)}return""});V=(await Promise.all(t)).join("")}catch(t){console.warn("[PROMPT_MANAGER] Dynamic client-side markdown injections error:",t)}let q="",G="";if(p&&p.length>0){if(q=p.map(t=>{const a=Array.isArray(t.linkedAccounts)?t.linkedAccounts:[];return`- **${t.perceivedName}** (Linked accounts: ${a.join(", ")||"none"})`}).join(`
...
${Ce}
    ...
    `.trim();return{...e,assembledSystemPrompt:De}}};export{Qe as PromptManagerModule};
