...
import { DEFAULT_PROVIDER_OPTIONS, DEFAULT_NEURAL_CORES, RUNTIME_DEFAULTS } from '@shared/constants';
...
export interface AppState {
  ...
  cortexRef: React.MutableRefObject<any | null>;
  ...
  idleMonologueEnabled: boolean;
  setIdleMonologueEnabled: React.Dispatch<React.SetStateAction<boolean>>;
...
export function useAppState() {
  ...
  const cortexRef = useRef<any | null>(null);
  ...
  const [state, setState] = useState<AgentState>({
    ...
    activePersonaId: safeLocalStorage.getItem('yuihime_active_persona') || 'auto',
    relation: {
      uid: 'anon',
      trust: 50,
      affection: 50,
  ...
  const [idleMonologueEnabled, setIdleMonologueEnabled] = useState(true);
  ...
  return {
    ...
    idleMonologueEnabled, setIdleMonologueEnabled,
