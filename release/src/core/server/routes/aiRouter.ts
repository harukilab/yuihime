...
export function registerAiRoutes(app: express.Express, db: any) {
  ...
  app.post("/api/tts/openai", express.json(), async (req, res) => {
    try {
      ...
      if (!finalApiKey) {
        ...
        finalApiKey = toSingleString(openaiSettings.apiKey) || process.env.OPENAI_API_KEY || process.env.OPENROUTER_API_KEY;
      }
  ...
  app.post("/api/tts/gemini", express.json(), async (req, res) => {
    try {
      ...
      if (!finalApiKey) {
        ...
        finalApiKey = toSingleString(geminiSettings.apiKey) || process.env.GEMINI_API_KEY;
      }
  ...
  app.post("/api/ai/verify", async (req, res) => {
    try {
      ...
      const apiKey = toSingleString(activeConfig.apiKey || activeConfig.api_key || activeConfig.apiToken || activeConfig.accessKeyId || activeConfig.token);
  ...
  app.post("/api/ai/diagnose", express.json(), async (req, res) => {
    try {
      ...
      const chosenModel = toSingleString(model || providerSettings.model);
