...
- **Character name via `${characterName}` placeholder**: NEVER hardcode the character's name (e.g. "Yui", "Yuihime") in prompt templates / cron boilerplate / user-facing strings. Use the `${characterName}` placeholder and resolve it with `injectCharacterName()` from `src/core/kernel/characterName.ts` (reads the configured name with `AI_NAME` fallback). Applies to `resolveCronJobPrompt`, scheduler boilerplate, speak/status_update tool text, and any LLM-facing instructions that reference the assistant's own name.
